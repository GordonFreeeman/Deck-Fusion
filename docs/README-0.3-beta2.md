...
- **ReShade HTTP 422:** update to beta2. The legacy pack now uses the repository's `legacy` branch. An unavailable optional pack is reported without blocking other effects. Required packs must download before Apply.
- **Graphics API:** Auto-detect checks the executable and local engine DLLs. Unity renderer launch flags take priority. Subnautica: Below Zero has a scoped DirectX 11 default hint. Multiple detected renderers require a dropdown selection; this is not a live GPU probe.
- **Runtime exit status 1:** open **Installer log** on Review or Windows runtimes. Beta2 fills missing display/session fields from the same user's Steam process and shows the actual installer output. Check the reported cause before retrying. Prefix snapshots are retained; graphics installation stops on failure. Update Protontricks through Discover if its Winetricks recipes are outdated.

The reported exit status alone does not identify the original runtime failure. Steam Deck/Proton runtime installation still needs device testing.
...
The package script checks ZIP paths, CRCs, source/bundle parity and the recorded file hashes. It writes the ZIP and a SHA-256 sidecar beside the source directory. Public release/tag naming is `v0.3-beta2`; npm uses the equivalent three-part version `0.3.0-beta2`.
